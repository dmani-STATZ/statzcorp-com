...
- **Models:** App models in `apps/*/models.py`. Surveys use abstract `ClassifiedModel` + `public_objects` manager (`apps/surveys/models.py`). Videos use `VideoAsset` + `published_objects` (`apps/videos/models.py`). Public home hero uses `HeroSlide` + `published_objects` (`apps/public/models.py`). `ContactRecipient` in `apps/contact/models.py` — admin-managed list of live notification recipients for the contact form; queried at send-time, not cached; `CONTACT_EMAIL_TO` is now a fallback-only comma-separated list.
...
- **Admin:** Models registered in `apps/contact/admin.py`, `apps/surveys/admin.py`, `apps/videos/admin.py`, and `apps/public/admin.py` (`HeroSlide`). Branding (`site_header` / `site_title` / `index_title`) is set in `apps.public.apps.PublicConfig.ready()`. Login template override: `templates/admin/login.html` (logo: `static/images/StatzCorpColorFINAL.png`). Superuser creation is a **manual operational step** (`python manage.py createsuperuser`) — never seed admin credentials in code, fixtures, or committed env files.
