...
- **Models:** App models in `apps/*/models.py`. Surveys use abstract `ClassifiedModel` + `public_objects` manager (`apps/surveys/models.py`). Videos use `VideoAsset` + `published_objects` (`apps/videos/models.py`). Public home hero uses `HeroSlide` + `published_objects` (`apps/public/models.py`).
