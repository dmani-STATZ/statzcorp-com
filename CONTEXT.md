...
| Supplier Portal — Supplier Contracts dashboard section | Done | Independent of remaining Phase 2 writes (still Not started below) and of any further profile work. `GET /suppliers/{cage_code}/contracts/` in `docs/supplier-portal-api-contract.md` §4.4; `get_supplier_contracts()` in `statzweb_client.py` (404 → `None`, same as `verify_supplier`); `present_supplier_contracts()` allowlists `contract_number`, `award_date`, `status`, `clin_number`, `nsn`, `due_date` only (no dollar values); `SupplierDashboardView` catches `StatzWebAPIError` and still renders the rest of the dashboard; `templates/supplier_portal/dashboard.html` renders one compact row-grouped `.supplier-portal-table` (one row per CLIN, contract number/award date/status carried on `rowspan` cells) to match the other dashboard sections. No new Django models or migrations — contract data is not stored in SQLite; STATZWeb is the system of record. |
