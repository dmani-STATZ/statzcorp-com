...
| Supplier Portal — Phase 1a shell (login/session only) | Done | `apps/supplier_portal/` — `SupplierPortalAccount` model (hashed password, lockout, session key `supplier_portal_account_id`, intentionally separate from `django.contrib.auth`); `supplier_portal:login`/`logout`/`dashboard` views; "Supplier Portal" tab added to `public:resources` jump-nav. Live supplier data landed in later rows (Phase 1b dashboard, then Supplier Contracts). |
| Supplier Portal — self-service passwords (staff out of the password chain) | Done | `apps/supplier_portal/` — request-access/forgot-password flow: supplier enters CAGE → sees masked on-file email → confirms → gets a signed 1-hour single-use set-password link (`tokens.py`, `emails.py`; token embeds a password-hash fragment so it dies on use). Cooldown 1 request/15 min per account. Admin has **no password field** — staff creates accounts with CAGE + contact email only; an admin action emails the same link. Passwords validated with `AUTH_PASSWORD_VALIDATORS`. Email delivery goes through STATZWeb's Graph-based `POST /send-email/` API (see row below), not Django's SMTP — see Known Issues #8 (resolved). |
| Supplier Portal — Phase 1b STATZWeb API client | Done | `apps/supplier_portal/statzweb_client.py` — signs every call with `X-API-Key` + HMAC-SHA256 (`X-Timestamp`/`X-Signature`) exactly matching STATZWeb's `suppliers/portal/auth.py` (independently confirmed byte-for-byte via a Cursor audit of the STATZWeb repo, 2026-07-20). `verify_supplier(cage_code)` calls STATZWeb's live `GET .../verify/` endpoint; `RequestAccessForm` (`forms.py`) now auto-provisions/refreshes `SupplierPortalAccount.contact_email` from that response instead of relying on staff-entered data — a CAGE code that exists in STATZWeb "just works" on the portal with no manual account-creation step. `send_email(to, subject, body_html)` calls STATZWeb's `POST /send-email/` (Microsoft Graph, always From `GRAPH_MAIL_SENDER_CONTRACT`; exactly the 3 keys `to`/`subject`/`body`, extra keys rejected 403) — `emails.py` routes all supplier-facing mail through this instead of Django's SMTP `send_mail`. Verified 2026-07-20 against a live local STATZWeb instance on real SQL Server: correct verify data returned, wrong HMAC secret correctly rejected, unknown CAGE code correctly returns nothing, send-email failure surfaces as a clean catchable error (Graph not configured in this local dev environment). STATZWeb's API contract: `docs/supplier-portal-api-contract.md`. |
| Supplier Portal — Phase 1b live dashboard and documents | Done | `supplier_portal:dashboard` fetches the authenticated supplier's company profile, addresses, contacts, certifications, classifications, and document metadata live from STATZWeb through `get_supplier()`. `apps/supplier_portal/presenters.py` is an explicit allowlist/security boundary before rendering. `get_document_download_url()` and `supplier_portal:document_download` provide a CAGE-scoped passthrough to short-lived document URLs without storing supplier data or Blob credentials locally. |
| Supplier Portal — Supplier Contracts dashboard section | Done | Independent of remaining Phase 2 writes (still Not started below) and of any further profile work. Live on the dashboard via `GET /suppliers/{cage_code}/contracts/` (`docs/supplier-portal-api-contract.md` §4.4); `get_supplier_contracts()` in `statzweb_client.py` (404 → `None`, same as `verify_supplier`); `present_supplier_contracts()` allowlists `contract_number`, `award_date`, contract-level `po_number`, `status`, and per-CLIN `clin_number`, `po_number`, `nsn`, `due_date` only (no dollar values). Contract-level and CLIN-level PO Number are distinct fields and are both rendered even when they match. `SupplierDashboardView` catches `StatzWebAPIError` and still renders the rest of the dashboard; `templates/supplier_portal/dashboard.html` renders one compact row-grouped `.supplier-portal-table` (one row per CLIN, contract number/award date/PO Number/status carried on `rowspan` cells) to match the other dashboard sections. No new Django models or migrations — contract data is not stored in SQLite; STATZWeb is the system of record. |
| Staff admin password reset | Done | `apps/accounts/` uses Django's built-in uid/token reset views and validation while routing reset email through STATZWeb's Graph-backed `send_email()` client. Responses remain address-enumeration safe and only active staff users are eligible. |
| Admin-editable Our Team page | Done | `TeamGroup` and `TeamMember` in `apps/public/` provide ordered publish controls, member reassignment, near-square photo validation, and a prefetched public rendering with no deploy step. |
| Admin-editable team banner | Done | `TeamPageBanner` in `apps/public/` uses Blob-or-local media storage and falls back to the legacy static team image when no banner is published. Banner/member image replacement and deletion remove the old object through Django's active storage backend. |
...
Browser
  → Django URLConf (statzcorp/urls.py)
       → apps.accounts   Django auth password-reset views → STATZWeb Graph email
       → apps.public     TemplateViews → templates/public/* (+ admin-managed hero/team media)
       ...
       → apps.supplier_portal  Session-based supplier login → live STATZWeb-backed dashboard + CAGE-scoped document download passthrough (no supplier business data stored locally)
       → /admin/         Django admin
...
- **Media / video files:** Local `FileSystemStorage` when `AZURE_CONNECTION_STRING` is unset; Azure Blob Storage (`storages.backends.azure_storage.AzureStorage`, GCCH-compatible connection string, container `media`) when set. Models: `VideoAsset` in `apps/videos/`; `HeroSlide`, `TeamPageBanner`, and `TeamMember` in `apps/public/`. Team banner/member images actively delete replaced or deleted files through the storage API; this cleanup intentionally does not apply to `HeroSlide` or `VideoAsset`. Django caps large admin uploads at `DATA_UPLOAD_MAX_MEMORY_SIZE` = 1 GB (`statzcorp/settings/base.py`). When no applicable published banner/slide rows exist, the public templates fall back to their legacy static images.
...
**Email:** Console backend forced in `statzcorp/settings/local.py`. Production uses SMTP settings from env (Office 365 host defaults in `base.py` / `.env.example`) for contact notifications. Supplier and staff password-reset email intentionally uses STATZWeb's Graph-backed `send_email()` API instead of Django SMTP.

**Auth for public site:** Anonymous public pages. Optional `Submission.user` FK if authenticated. Entra ID / MSAL packages are commented out in `requirements.txt` (“add when ready for internal views”) — not active. Staff/admin accounts use `django.contrib.auth`; `apps/accounts` adds an enumeration-safe self-service password reset flow using Django's standard tokens and STATZWeb Graph email. Supplier Portal (`apps/supplier_portal`) is a third, separate identity system: session key `supplier_portal_account_id` against `SupplierPortalAccount`, deliberately not `django.contrib.auth` — a supplier login is a company (CAGE code), not a staff/admin identity. Supplier passwords are entirely self-service (signed emailed set-password links); staff never sets, sees, or resets a supplier password — by design (“chain of custody” decision, 2026-07-20). The on-file `contact_email` is sourced live from STATZWeb's `verify/` endpoint (`statzweb_client.py`) on every request-access attempt, auto-provisioning/refreshing the local `SupplierPortalAccount` — no separate staff account-creation step for a supplier that already exists in STATZWeb.
...
8. **~~Supplier Portal emails to external recipients unverified via SMTP~~ (resolved differently than expected).** GCCH direct-send (port 25) indeed can't reach external supplier addresses reliably — but instead of fixing SMTP, supplier-facing portal email was rerouted entirely: `apps/supplier_portal/emails.py` now calls STATZWeb's `POST /send-email/` (Microsoft Graph, decision 2026-07-20), not Django's `send_mail`. Django's SMTP config (`base.py`/`.env.example`) is unaffected and still used for the internal contact form. Not yet verified against a real external address in production — Graph delivery itself should be tested before launch, same caution as before, different mechanism.
...
10. **Production STATZWeb (`https://statzweb.azurewebsites.us`) returns HTTP 403 with no contract error envelope from statzcorp-com.** The live Supplier Portal dashboard now depends on this production path. Until statzcorp-com's outbound IP is added to STATZWeb's App Service Access Restrictions allowlist, every production dashboard load intentionally degrades to the supplier-facing unavailable state with HTTP 200 rather than raising a 500. The staff diagnostic distinguishes an access restriction/WAF response from a missing `/api/supplier-portal/` base-path prefix. Kudu/SSH probe: `python manage.py statzweb_ping`.
11. **Supplier address shape is not yet confirmed.** The exact `addresses.*` keys from STATZWeb's `contracts.Address` model remain open question #3 in `docs/supplier-portal-api-contract.md`. The presenter renders address values defensively in a stable order and excludes identifier/audit keys; tighten this to a strict allowlist once the upstream shape is confirmed.
12. **~~Scroll-reveal animation blanked the Supplier Portal dashboard~~ (resolved 2026-08-24).** `.animate-on-scroll` sets `opacity: 0` until `static/js/main.js` adds `.in-view` via IntersectionObserver. The observer used `threshold: 0.12`, which an element taller than the viewport can never satisfy — once the dashboard grew (profile + contracts), the whole card stayed invisible while its HTML was present in view-source. Fixes: threshold is now `0` with a `rootMargin` offset; the portal card no longer uses `animate-on-scroll`; the hide-by-default rules moved from a `<style>` block in `templates/base.html` into `static/css/style.css` scoped to `.js`, so a failed/blocked `main.js` can no longer blank a page. Watch for this on any future long page that opts into the class.
13. **~~Supplier API diagnostic exposed from the login page~~ (resolved).** `ApiConnectionTestView` is gated by active Django staff authentication, returns 403 to logged-out and non-staff users, and is no longer linked from the public Supplier Portal login page.
