...
12. **~~Supplier API diagnostic exposed from the login page~~ (resolved).** `ApiConnectionTestView` is gated by active Django staff authentication, returns 403 to logged-out and non-staff users, and is no longer linked from the public Supplier Portal login page.
