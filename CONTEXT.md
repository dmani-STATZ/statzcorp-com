...
10. **Production STATZWeb (`https://statzweb.azurewebsites.us`) not yet reachable from statzcorp-com.** Believed to be an Azure App Service network-level Access Restriction (IP allowlist) on the STATZWeb App Service — not an app-level block; `suppliers/portal/` already exempts `/api/supplier-portal/` from STATZWeb's own `LoginRequiredMiddleware` (`STATZWeb/middleware.py:51`), confirmed by reading the code. Fix (pending): add statzcorp-com's outbound IP(s) to STATZWeb's Access Restrictions allowlist. Local `.env` values for `SUPPLIER_PORTAL_API_BASE_URL` currently point at a local STATZWeb dev instance (`127.0.0.1:8001`), not production — production needs its own App Service Application Setting with the real base URL once network access is sorted out.
